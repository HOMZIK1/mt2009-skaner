# MT Market Scanner GUI (clean build) - v1.0.2
import sys, os, re, json, csv, hashlib, time
...
def parse_fields(txt: str):
    lines = [l.strip() for l in txt.splitlines() if l.strip()]
    if not lines:
        return None, None, None
    name = None
    for l in lines[:4]:
        if not re.search(r"(Od Poziomu|Cena|Yang|kup przedmiot|kup wszystkie podobne)", l, re.I):
            name = l
            break
    if name is None:
        name = lines[0]
...
class OcrWorker(QtCore.QThread):
    ...
    def __init__(self, cfg):
        ...
        self.log_dir = os.path.abspath(self.cfg.get("log_dir", "./logs_market"))
    ...
    def hash_entry(self, name, price):
        return hashlib.md5(f"{name}|{price}".encode("utf-8")).hexdigest()
    def should_log(self, h):
        t = time.time()
        dedup_s = float(self.cfg.get("dedup_seconds", 2.0))
        while self.recent and (t - self.recent[0][1]) > dedup_s:
            self.recent.popleft()
        for hh, ts in self.recent:
            if hh == h:
                return False
        self.recent.append((h, t))
        return True
    def set_running(self, state: bool):
        self.running = state
        self.status.emit(self.running)
    def run(self):
        fps = float(self.cfg.get("fps", 4.0))
        min_dt = 1.0 / max(0.5, fps)
        with mss.mss() as sct:
            while not self.stop_flag:
                ...
                x, y = pos.x, pos.y
                roi_w = int(self.cfg.get("roi_width", 520))
                roi_h = int(self.cfg.get("roi_height", 300))
                mx = int(self.cfg.get("margin_x", 30))
                my = int(self.cfg.get("margin_y", 30))
                mon = sct.monitors[0]
                left = max(0, x - mx)
                top = max(0, y - my)
                if left + roi_w > mon["width"]:
                    left = mon["width"] - roi_w
                if top + roi_h > mon["height"]:
                    top = mon["height"] - roi_h
                bbox = {"top": int(top), "left": int(left), "width": roi_w, "height": roi_h}
                img = np.array(sct.grab(bbox))[:, :, :3]
                th = preprocess_for_ocr(img)
                self.frameReady.emit(th)
                lang = self.cfg.get("language", "pol+eng")
                config = r'--oem 3 --psm 6 -l {}'.format(lang)
                try:
                ...
                if not txt or len(txt) < 4:
                    ...
                    continue
                name, price, unit_price = parse_fields(txt)
                if name and price:
                    kws = [k.strip().lower() for k in self.cfg.get("keywords", []) if k.strip()]
                    import re as _re
                    passed = any(k in name.lower() for k in kws) or                              _re.search(r"ksi[eę]ga\s+umiej[eę]tno[sś]ci", name, _re.I) or                              _re.search(r"pergamin", name, _re.I) or                              _re.search(r"kamie[nń]\s*duszy.*\+4", name, _re.I)
                    if not passed:
                        dt = time.time() - t0
                        if dt < min_dt:
                            time.sleep(min_dt - dt)
                    ...
                    if self.should_log(h):
                        ...
                        with open(self.csv_path, "a", newline="", encoding="utf-8") as f:
                            csv.writer(f).writerow([now_ts(), name, price, unit_price if unit_price else "", category, txt])
                        self.itemLogged.emit({"timestamp": now_ts(), "name": name, "price": price, "unit_price": unit_price, "category": category})
                dt = time.time() - t0
                if dt < min_dt:
                    time.sleep(min_dt - dt)
...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        if self.cfg.get("auto_update_on_start", True):
            self.try_update()
        ...
        self.count_total = 0
        self.counts = {"book":0, "scroll":0, "soulstone+4":0, "upgrade":0, "other":0}
        self.tabs = QtWidgets.QTabWidget()
        self.setCentralWidget(self.tabs)
        self.tab_view = QtWidgets.QWidget()
        self.tab_settings = QtWidgets.QWidget()
        self.tabs.addTab(self.tab_view, "Podgląd")
        ...
        if HAS_KEYBOARD:
            try:
                keyboard.add_hotkey("F8", self.toggle_run)
                keyboard.add_hotkey("F9", self.close)
            except Exception:
                pass
    def try_update(self):
    ...
    def build_view_tab(self):
        ...
        self.preview_label = QtWidgets.QLabel("Podgląd OCR")
        ...
        self.last_text = QtWidgets.QPlainTextEdit()
        self.last_text.setReadOnly(True)
        self.last_text.setMaximumHeight(110)
        self.last_text.setStyleSheet("background-color:#1b1b1b; color:#c8c8c8; border:1px solid #333;")
        left.addWidget(QtWidgets.QLabel("Ostatni tekst OCR:"))
        left.addWidget(self.last_text)
        layout.addLayout(left, stretch=2)
        right = QtWidgets.QVBoxLayout()
        self.status_label = QtWidgets.QLabel("STATUS: STOP")
        ...
        self.counter_label = QtWidgets.QLabel("Zalogowano (sesja): 0")
        self.counter_label.setStyleSheet("color:#ffd27f;")
        right.addWidget(self.counter_label)
        self.cat_label = QtWidgets.QLabel("Księgi: 0 | Ulepszacze: 0 | Kamienie+4: 0 | Inne: 0")
        self.cat_label.setStyleSheet("color:#ffd27f;")
        right.addWidget(self.cat_label)
        self.last_read_label = QtWidgets.QLabel("Ostatni odczyt: —")
        self.last_read_label.setStyleSheet("color:#e6e6e6;")
        right.addWidget(self.last_read_label)
        right.addWidget(QtWidgets.QLabel("Ostatnie logi:"))
        self.logs_list = QtWidgets.QListWidget()
        self.logs_list.setStyleSheet("background-color:#1b1b1b; color:#c8c8c8; border:1px solid #333;")
        right.addWidget(self.logs_list, stretch=1)
        btns = QtWidgets.QHBoxLayout()
        self.btn_start = QtWidgets.QPushButton("Start (F8)")
        self.btn_start.clicked.connect(self.toggle_run)
        self.btn_roi = QtWidgets.QPushButton("Ustaw obszar skanu")
        self.btn_roi.clicked.connect(self.set_roi_area)
        self.btn_update = QtWidgets.QPushButton("Sprawdź aktualizacje")
        self.btn_update.clicked.connect(self.try_update)
        self.btn_close = QtWidgets.QPushButton("Zamknij (F9)")
        self.btn_close.clicked.connect(self.close)
        for b in (self.btn_start, self.btn_roi, self.btn_update, self.btn_close):
            b.setStyleSheet("background:#2a2a2a; color:#ffd27f; padding:8px; border:1px solid #444; border-radius:6px;")
        btns.addWidget(self.btn_start); btns.addWidget(self.btn_roi); btns.addWidget(self.btn_update); btns.addWidget(self.btn_close)
    ...
    def build_settings_tab(self):
        ...
        self.in_title = QtWidgets.QLineEdit(self.cfg.get("window_title_filter",""))
        ...
        form.addRow("FPS:", self.in_fps)
        form.addRow("Szerokość ROI:", self.in_roi_w)
        form.addRow("Wysokość ROI:", self.in_roi_h)
        form.addRow("Margines X:", self.in_mx)
        form.addRow("Margines Y:", self.in_my)
        form.addRow("Antyduplikacja [s]:", self.in_dedup)
        form.addRow("Tytuł okna gry:", self.in_title)
        form.addRow("Ścieżka Tesseract:", self.in_tess)
        form.addRow("Folder logów:", self.in_logdir)
        form.addRow("Język OCR (np. pol+eng):", self.in_lang)
        btn_save = QtWidgets.QPushButton("Zapisz ustawienia")
        btn_save.setStyleSheet("background:#2a2a2a; color:#ffd27f; padding:8px; border:1px solid #444; border-radius:6px;")
    ...
    def save_settings(self):
        ...
        QtWidgets.QMessageBox.information(self, "Zapisano", "Ustawienia zapisane.")
    def toggle_run(self):
        self.worker.set_running(not self.worker.running)
        self.update_status_ui()
    def on_status(self, running):
        self.update_status_ui()
    def update_status_ui(self):
        if self.worker.running:
            self.status_label.setText("STATUS: LOGOWANIE")
            self.status_label.setStyleSheet("font-weight:bold; padding:8px; border-radius:6px; background:#1d5a2a; color:#ffd27f;")
            self.btn_start.setText("Stop (F8)")
        else:
            self.status_label.setText("STATUS: STOP")
            self.status_label.setStyleSheet("font-weight:bold; padding:8px; border-radius:6px; background:#5a1d1d; color:#ffd27f;")
            self.btn_start.setText("Start (F8)")
    def on_frame_ready(self, gray_img):
        h, w = gray_img.shape
        qimg = QtGui.QImage(gray_img.data, w, h, w, QtGui.QImage.Format_Grayscale8)
    ...
    def on_last_text(self, txt):
        self.last_text.setPlainText(txt)
    def on_item_logged(self, data):
        self.count_total += 1
        self.counts[data["category"]] = self.counts.get(data["category"], 0) + 1
        ...
        self.last_read_label.setText(f"Ostatni odczyt: {data['name']} - {data['price']} Yang")
        self.logs_list.insertItem(0, f"[{data['timestamp']}] {data['name']} - {data['price']}Y")
...
def main():
    ...
    app.setStyleSheet('QMainWindow { background-color: #0f0f0f; } QLabel { color: #f2e7c9; } QTabWidget::pane { border: 1px solid #3b2d12; } QTabBar::tab { background: #201a10; color: #d9c089; padding: 8px 12px; border: 1px solid #3b2d12; } QTabBar::tab:selected { background: #2a2316; } QPushButton { background: #2a2a2a; color: #ffd27f; padding: 8px; border: 1px solid #444; border-radius: 6px; } QPushButton:hover { background: #333; } QLineEdit, QSpinBox, QDoubleSpinBox, QListWidget, QPlainTextEdit { background: #1b1b1b; color: #c8c8c8; border: 1px solid #333; }')
    w = MainWindow()
    w.show()
    w.worker.start()
    sys.exit(app.exec_())
